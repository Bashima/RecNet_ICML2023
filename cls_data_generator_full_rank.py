...
class DataGenerator(object):
    def __init__(
        ...
        self._actual_feat_dir = "./DCASE2021_SELD_dataset/seld_feat_label_4_ch_minmax/mic_dev_norm/"
    ...
    def generate(self):
        ...
        if self._is_eval:
            ...
            for i in range(self._nb_total_batches):
                ...
                # Read one batch size from the circular buffer
                feat = np.zeros((self._feature_batch_seq_len, self._nb_mel_bins * self._nb_ch))
                feat_act = np.zeros((self._feature_batch_seq_len, self._nb_mel_bins * self._nb_ch))
                for j in range(self._feature_batch_seq_len):
                    feat[j, :] = self._circ_buf_feat.popleft()
                for j in range(self._feature_batch_seq_len):
                    feat_act[j, :] = self._circ_buf_act_feat.popleft()
                feat = np.reshape(feat, (self._feature_batch_seq_len, self._nb_ch, self._nb_mel_bins))
                feat_act = np.reshape(feat_act, (self._feature_batch_seq_len, self._nb_ch, self._nb_mel_bins))
                if self._multi_accdoa is True:
                    label = np.zeros(
